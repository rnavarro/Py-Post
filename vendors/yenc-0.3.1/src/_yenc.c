 ...
 *=============================================================================
 *
 * $Id: _yenc.c,v 1.2 2004/02/18 23:06:14 cvs Exp $
 * 
 */
...
static void crc_init(Crc32 *, int);
static void crc_update(Crc32 *, int);
static Bool readable(FILE *);
static Bool writable(FILE *);
void init_yenc(void);
static int encode_buffer(char *, char *, int, Crc32*, int *);
static int decode_buffer(char *, char *, int, Crc32*, Bool *);
...
static void crc_init(Crc32 *crc, int value)
{
	crc->crc = value;
	crc->bytes = 0UL;
}

static void crc_update(Crc32 *crc ,int c)
{
	crc->crc=crc_tab[(crc->crc^(c&0x000000ffL))&0xffL]^((crc->crc>>8L)&0xffffffL);
	crc->bytes++;
}
		...
		)
{
	...
	fflush(outfile);
	return Py_BuildValue("(l,i)",encoded,(unsigned int) (crc.crc ^ 0xFFFFFFFF));
}
