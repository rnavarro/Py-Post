...
#
def post(config, fprint, loc):
	articles = []
	
	global locations, totalFiles, fingerprint, jobs
	locations = loc
	fingerprint = fprint
	totalFiles = len(os.listdir(locations['path']+locations['workingDirectory']))
	jobs = Queue.Queue(config['connections'])
	...
	for filename in os.listdir(locations['path']+locations['workingDirectory']):
		thread = postThread(config, filename, totalFiles)
		thread.start()
		jobs.put(thread, True)
	
	# Wait for threads to finish and capture output
	while len(articles) < totalFiles:
		thread = jobs.get(True)
		thread.join()
		articles.append(thread.get_result())
...
class postThread(threading.Thread):
	def __init__(self, config, filename, totalFiles):
		self.conn = nntplib.NNTP(config['hostname'], config['port'], config['username'], config['password'])
		self.config = config
	...
	def run(self):
		try:
			...
			curFile = os.listdir(locations['path']+locations['workingDirectory']).index(self.filename)+1
			filePath = locations['path']+locations['workingDirectory']+os.sep+self.filename
			...
			for part in range(1,parts+1):
				print('File ['+str(curFile)+'/'+str(self.totalFiles)+'] - "'+self.filename+'" - Posting Part '+str(part)+'/'+str(parts))
				...
				if(self.config['fingerprint']): subject += '['+fingerprint+'] '
				subject += '['+str(curFile)+'/'+str(totalFiles)+'] - "'+self.filename+'" yEnc ('+str(part)+'/'+str(parts)+')'	

				msg = MIMEText(temp_out.getvalue())
				msg['Subject'] = subject
				if(debug): print('Subject: '+msg['Subject'])
				msg['From'] = '<'+self.config['name']+'> '+self.config['email']
				msg.add_header('Newsgroups', self.config['groups'])
